...
const dbg = debug("genaid:api")

/**
 * Runs a GenAID script with the given files and options.
 ...
 * @param options GenAID generation options, including optional environment variables, an abort signal, and additional options. The options may include a label for the worker thread.
...
export async function run(
    ...
    const filename =
        typeof __filename === "undefined"
            ? join(dirname(fileURLToPath(import.meta.url)), "genaid.cjs") // ignore esbuild warning
            : __filename
    dbg(`start ${filename}`)
