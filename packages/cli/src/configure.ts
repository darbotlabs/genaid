...
export async function configure(options: { provider?: string }) {
    while (true) {
            ...
            : await select({
                  ...
                  choices: MODEL_PROVIDERS.filter(({ hidden }) => !hidden).map(
                      (provider) => ({
                          ...
                          value: provider,
                          description: `'${provider.id}': https://darbotlabs.github.io/genaid/getting-started/configuration#${provider.id}`,
                      })
                  ),
              })
        if (!provider) break
        ...
        logVerbose(
            `- docs: https://darbotlabs.github.io/genaid/getting-started/configuration#${provider.id}`
        )
        while (true) {
