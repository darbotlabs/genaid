...
import { promptyParse, promptyToGenAID } from "../../core/src/prompty"
...
const dbg = genaidDebug("cli:parse")
 ...
 */
export async function prompty2genaid(
    files: string[],
    options: { out: string }
) {
    const { out } = options
    ...
    for (const f of fs) {
        ...
        const script: string = promptyToGenAID(doc)
