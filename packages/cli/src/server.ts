...
import { genaidDebug } from "../../core/src/debug"
import { startProjectWatcher } from "./watch"
import { buildProject } from "./build"
const dbg = genaidDebug("server")
...
export async function startServer(
    ...
    const corsOrigin = options.cors || process.env.GENAID_CORS_ORIGIN
    const apiKey = options.apiKey || process.env.GENAID_API_KEY
    const serverHost = options.network ? "0.0.0.0" : "127.0.0.1"
    const remote = options.remote
    const dispatchProgress = !!options.dispatchProgress
    const disableWatch = process.env.GENAID_DISABLE_WATCH === "1"
    ...
    const watcher = disableWatch ? undefined : await startProjectWatcher({})
    const projectProvider = {
        async project() {
            if (watcher) return watcher.project()
            return buildProject()
        },
    }
    ...
    const scriptList = async () => {
        ...
        const project = await projectProvider.project()
    ...
    const imageRx =
        /^\/\.genaid\/(images|runs\/.*?)\/[a-z0-9]{12,128}\.(png|jpg|jpeg|gif|svg)$/
...
window.vscodeWebviewPlaygroundNonce = ${JSON.stringify(nonce)};
    ...
    httpServer.listen(port, serverHost, () => {
        console.log(`GenAID server v${CORE_VERSION}`)
