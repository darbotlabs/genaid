/**
 * CLI entry point for the GenAID tool, providing various commands and options
 * for interacting with scripts, parsing files, testing, and managing cache.
 */
import debug from "debug"
const dbg = debug("genaid:cli")
...
export async function cli() {
    ...
    program.on("option:debug", (c: string) =>
        debug.enable(c === DEBUG_SCRIPT_CATEGORY ? c : `genaid:${c}`)
    )
    ...
    const run = program
        .command("run")
        .description("Runs a GenAID script against files.")
    ...
    addModelOptions(run) // Add model options to the command
        ...
        .option(
            "--vars <namevalue...>",
            "variables, as name=value, stored in env.vars. Use environment variables GENAID_VAR_name=value to pass variable through the environment"
        )
        .option(
            "-rr, --run-retry <number>",
    ...
    const convert = program
        .command("convert")
        .description(
            "Converts file through a GenAID script. Each file is processed separately through the GenAID script and the LLM output is saved to a <filename>.genai.md (or custom suffix)."
        )
        .arguments("<script> [files...]")
    ...
    scripts
        ...
        .option(
            ...
            "Write GitHub Copilot custom instructions for better GenAID code generation"
        )
    ...
    const serve = program
        .command("serve")
        .description("Start a GenAID local web server")
        .option(
            "-p, --port <number>",
    ...
    parser
        .command("prompty")
        .description("Converts .prompty files to genaid")
        .argument("<file...>", "input JSONL files")
        .option("-o, --out <string>", "output folder")
        .action(prompty2genaid) // Action to convert prompty files
    parser
        .command("jinja2")
