...
export async function runScriptInternal(
    ...
    logInfo(`genaid: ${scriptId} (run: ${runId})`)
    ...
    if (teamsMessage && result.text) {
        ...
        const channelURL =
            process.env.GENAID_TEAMS_CHANNEL_URL ||
    ...
    if (result.status === "success") logInfo(`genaid: ${result.status}`)
    else if (result.status === "cancelled")
        logWarn(`genaid: ${result.status}`)
    else logError(`genaid: ${result.status}`)
    stats.log()
