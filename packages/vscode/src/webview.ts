...
<head>
    ...
    <title>GenAID View Holder</title>
...
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>GenAID Script Runner</title>
    ...
    <script nonce="${nonce}">
        ...
        window.genaid = ${JSON.stringify({ apiKey: sessionApiKey, base: authority })};
    </script>
</head>
<body>
...
export function activeWebview(state: ExtensionState) {
    ...
    subscriptions.push(
        registerCommand("genaid.request.open.view", async () => {
