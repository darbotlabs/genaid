...
export async function activateChatParticipant(state: ExtensionState) {
    ...
    const participant = vscode.chat.createChatParticipant(
        ...
        async (
            ...
            const mdHelp = () =>
                md(
                    `\n\n[Docs](https://darbotlabs.github.io/genaid/reference/vscode/github-copilot-chat/) | [Samples](https://darbotlabs.github.io/genaid/samples/)\n`
                )
            const mdEmpty = () =>
                md(
                    `\n😞 Oops, I could not find any genaid. [Create a new script](command:genaid.prompt.create)?\n`,
            ...
            if (command === "list") {
                if (templates.length) {
                    md("Use `@genaid /run <scriptid> ...` with:\n")
                    mdTemplateList()
                } else {
                    mdEmpty()
                }
            ...
            if (!template) {
                if (scriptid === "")
                    md(`😓 Please specify a genaid to run.\n`)
                else
                    md(
                        `😕 Oops, I could not find any genaid matching \`${scriptid}\`.\n`
                    )
