...
const noRequest = `
No GenAID request found yet. Please run a GenAID.
`
const noResponse = `
Waiting for GenAID response...
`
    ...
    implements vscode.TextDocumentContentProvider
{
    ...
    async provideTextDocumentContent(
        ...
        const wrap = (md: string) => {
            ...
            return `${computing ? `> **GenAID run in progress.**\n` : ""} 
${prettifyMarkdown(md)}    
            `
        }
...
export function activateMarkdownTextDocumentContentProvider(
    ...
    subscriptions.push(
        ...
        registerCommand("genaid.request.open", async (id: string) => {
            const uri = infoUri(id || REQUEST_TRACE_FILENAME)
            await showMarkdownPreview(uri)
        }),
        registerCommand("genaid.request.open.trace", () => {
        ...
        registerCommand("genaid.request.open.output", async () => {
            state.dispatchAIRequestChange()
            return vscode.commands.executeCommand(
                "genaid.request.open",
                REQUEST_OUTPUT_FILENAME
