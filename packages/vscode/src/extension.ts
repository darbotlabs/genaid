...
export async function activate(context: ExtensionContext) {
    ...
    context.subscriptions.push(
        registerCommand("genaid.server.start", async () => {
            await state.host.server.start()
        }),
        registerCommand("genaid.server.stop", async () => {
            await state.host.server.close()
        }),
        registerCommand("genaid.server.show", async () => {
            await state.host.server.show()
        }),
        registerCommand("genaid.request.abort", async () => {
            await state.cancelAiRequest()
            await vscode.window.showInformationMessage(
                `${TOOL_NAME} - request aborted.`
            )
        }),
        registerCommand("genaid.request.status", async () => {
