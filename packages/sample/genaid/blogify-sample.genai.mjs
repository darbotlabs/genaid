script({
    files: "packages/sample/genaid/readme-updater.genai.mts",
...
$`Create a blog post file that explains the GenAID source code in FILE.
...
- add a section that explains how to run the script with the genaid cli. Do not explain how to install the cli, link to the documentation instead.
...
- include canonical_url to genaid blog at https://darbotlabs.github.io/genaid/blog/<topic>
...
- when invoking the CLI, prefer using the script filename instead of the full path : "genaid run <filename_without_genai_extension>"
- if the script imports code from "genaid/runtime", explain what those imports do. The source is in file packages/cli/src/runtime.ts
...
- type definition: genaid/genaid.d.ts
- samples: packages/sample/genaid/*.genai.*s
- online documentation: https://darbotlabs.github.io/genaid/ . The routing is docs/src/content/docs/**/<route>.md* becomes https://darbotlabs.github.io/genaid/<route>.
- use links to online documentation
- github repository: https://github.com/darbotlabs/genaid . "main" is the default branch.
