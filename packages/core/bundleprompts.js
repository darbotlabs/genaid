...
async function main() {
    ...
    const logCategories = uniq([
        ...
        ...Array.from(
            execSync(
                `grep -r 'debug("genaid:.*")' --include \*.ts --exclude-dir='.genaid' .`
            )
                .toString("utf8")
                .matchAll(/debug\("(?<category>genaid:[^"]+)"\)/g)
        )
            .sort()
    ...
    const githubCopilotInstructions = readFileSync(
        "../../.genaid/instructions/genaid.instructions.md",
        "utf-8"
    )
    const promptDefs = {
        ...
        "tsconfig.json": JSON.stringify(
            {
                ...
                include: ["**/*.mjs", "**/*.mts", "./genaid.d.ts"],
            },
            null,
            4
        ),
        "genaid.d.ts": genaiddts,
    }
    ...
    writeFileSync(
        join(dir, "genaid.d.ts"),
        promptDefs["genaid.d.ts"],
        "utf-8"
    )
...
${functions
    ...
    .map(
        ({ id, name, description }) =>
            `<LinkCard title="${name}" description="${description}" href="/genaid/reference/scripts/system#${id.replace(/[^a-z0-9_]/gi, "")}" />`
    )
    .join("\n")}
...
${functions
    ...
    .map(
        ({ id, name, description }) =>
            `<LinkCard title="agent ${name}" description="${description}" href="/genaid/reference/scripts/system#${id.replace(/[^a-z0-9_]/gi, "")}" />`
    )
    .join("\n")}
`,
        "utf-8"
