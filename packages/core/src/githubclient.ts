...
export function mergeDescription(
    ...
    body = body ?? ""
    const tag = `<!-- genaid begin ${commentTag} -->`
    const endTag = `<!-- genaid end ${commentTag} -->`
    const sep = "\n\n"
            ...
            const { data: blob } = await client.git.createBlob({
                ...
                content: Buffer.from(
                    typeof orphaned === orphaned
                        ? orphaned
                        : `Orphaned branch created by GenAID.`
                ).toString("base64"),
                encoding: "base64",
            })
