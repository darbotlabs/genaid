...
const dbg = genaidDebug("dirs")

/**
 * Constructs a resolved file path within the `.genaid` directory of the project.
 *
 * @param segments - Additional path segments to append to the `.genaid` directory path.
 * @returns The resolved path as a string.
 */
export function dotGenaidPath(...segments: string[]) {
...
/**
 * Ensures the existence of the base `.genaid` directory.
 *
 * This function creates the `.genaid` directory at the root of the project folder
 ...
 */
export async function ensureDotGenaidPath() {
    const dir = dotGenaidPath()
    await ensureDir(dir)
...
/**
 * Generates the directory path for a specific run under the `.genaid` folder structure.
 *
 ...
 * `.genaid` folder, appends a timestamped folder name, ensures the
 * directory's existence, and returns the directory path.
 */
export async function createVideoDir() {
    const dir = dotGenaidPath("videos", friendlyDate())
    await ensureDir(dir)
    return dir
}
 ...
 * predefined `STATS_DIR_NAME` within the `.genaid` folder. It ensures
 * the directory exists by creating it if necessary.
 */
export async function createStatsDir() {
    const statsDir = dotGenaidPath(STATS_DIR_NAME)
