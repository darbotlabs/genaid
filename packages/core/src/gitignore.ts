...
const dbg = genaidDebug("files:gitignore")
 ...
 * Combines multiple .gitignore files (.gitignore, .gitignore.genaid, legacy .gitignore.genai, and .genaidignore)
...
export async function createGitIgnorer(): Promise<GitIgnorer> {
    const gitignores = [
        ...
        await tryReadText(GIT_IGNORE_GENAID),
        await tryReadText(GIT_IGNORE_GENAI),
        await tryReadText(GENAIDIGNORE),
    ].filter((g) => !!g)
