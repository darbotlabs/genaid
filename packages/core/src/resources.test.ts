...
describe("resources", async () => {
    ...
    await test("should resolve https URL to raw content", async () => {
        const url =
            "https://raw.githubusercontent.com/darbotlabs/genaid/refs/heads/main/package.json"
        const result = await tryResolveResource(url)

        assert(result)
        assert.equal(result.files.length, 1)
        assert(result.files[0].content)
        assert(result.files[0].content.includes("GenAID"))
    })

    await test("should adapt GitHub blob URLs to raw URLs", async () => {
        const url =
            "https://github.com/darbotlabs/genaid/blob/main/package.json"
        const result = await tryResolveResource(url)
