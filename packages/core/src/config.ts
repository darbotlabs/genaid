...
async function resolveGlobalConfiguration(
    ...
    if (process.env.GENAID_ENV_FILE) {
        dbg(
            `adding env file from environment variable: '${process.env.GENAID_ENV_FILE}'`
        )
        config.envFile = [
            ...(config.envFile || []),
            process.env.GENAID_ENV_FILE,
        ]
    }
    ...
    if (!config.envFile?.length) {
        ...
        config.envFile = [
            join(homedir(), DOT_ENV_GENAID_FILENAME),
            DOT_ENV_GENAID_FILENAME,
            DOT_ENV_FILENAME,
        ]
    }
