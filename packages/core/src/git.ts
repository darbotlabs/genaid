...
export class GitClient implements Git {
    ...
    private async resolveExcludedPaths(options?: {
        ...
        const dp = (await tryReadText(GIT_IGNORE_GENAID))?.split("\n")
        const dpLegacy = (await tryReadText(GIT_IGNORE_GENAI))?.split("\n")
        dbg(`reading GENAIDIGNORE file`)
        const dp2 = (await tryReadText(GENAIDIGNORE))?.split("\n")
        const ps = [
            ...arrayify(ep, { filterEmpty: true }),
            ...arrayify(dp, { filterEmpty: true }),
            ...arrayify(dpLegacy, { filterEmpty: true }),
