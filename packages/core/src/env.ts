...
const dbg = debug("genaid:config:env")
 ...
 *   - GENAID_DEFAULT_MODEL: Specifies the default model for the "large" alias.
 *   - GENAID_DEFAULT_TEMPERATURE: Sets the default temperature for the model, if defined.
 *   - GENAID_DEFAULT_[ID]_MODEL or GENAID_MODEL_[ID]: Configures aliases for specific model IDs.
 */
export async function parseDefaultsFromEnv(env: Record<string, string>) {
    dbg(`parsing process.env`)
    // legacy
    if (env.GENAID_DEFAULT_MODEL) {
        dbg(`found GENAID_DEFAULT_MODEL: ${env.GENAID_DEFAULT_MODEL}`)
        runtimeHost.setModelAlias("env", "large", env.GENAID_DEFAULT_MODEL)
    }

    const rx =
        /^GENAID(_DEFAULT)?_((?<id>[A-Z0-9_\-]+)_MODEL|MODEL_(?<id2>[A-Z0-9_\-]+))$/i
    ...
    const t = normalizeFloat(env.GENAID_DEFAULT_TEMPERATURE)
    if (!isNaN(t)) {
        dbg(`parsed GENAID_DEFAULT_TEMPERATURE: ${t}`)
