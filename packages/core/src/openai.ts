...
const dbg = debug("genaid:openai")
const dbgMessages = debug("genaid:openai:msg")
...
export async function OpenAITranscribe(
    ...
    try {
        ...
        const fetchImpl = await createFetch({ trace, cancellationToken })
        const res = await fetchImpl(url, freq as any)
