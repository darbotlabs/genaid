...
export function traceCliArgs(
    ...
    trace.details(
        "🤖 automation",
        `Use the command line interface [run](https://darbotlabs.github.io/genaid/reference/cli/run/) to automate this task:
...
`
    ...
    if (arrayify(template.tests)?.length) {
        trace.details(
            "🧪 testing",
            `
Use the command line interface [test](https://darbotlabs.github.io/genaid/reference/cli/test) to run the tests for this script:

\`\`\`sh
npx --yes genaid test ${template.id}
\`\`\`
`
        )
    }
