...
if (!topic) {
    ...
    const res = await runPrompt(
        (_) => {
            _.$`You are a blog writer expert on GenAID (https://darbotlabs.github.io/genaid).
# Task

Generate a blog post topic on the topic of writing and using a GenAID script.
...
let snippet: string | Fenced
{
    const { text, fences, error } = await runPrompt(
        (_) => {
            _.defTool(
                "genaid_typecheck",
                "Validates the syntax and type checks a GenAID source code",
                {
                    source: {
                        type: "string",
                        description: "The GenAID javascript source code",
                    },
                },
                async ({ source }) => {
                    ...
                    return await host.exec(
                        "node packages/cli/built/genaid.cjs scripts compile temp/blog-post"
                    )
                }
            )

            _.def("TOPIC", topic)
            _.$`You are a JavaScript developer expert on GenAID (https://darbotlabs.github.io/genaid).

        # Task
        
        - Generate a GenAID source code that implements the idea described in TOPIC.
        - Validate syntax and checking with genaid_typecheck.
        - Respond ONLY with the JavaScript source code. Do NOT fence code in markdown. Do not add text around code.
        - do not import "genaid" module, it is ambient.
        
        # Information
        
        Use these files to help you generate a topic for the blog post.
 
        - the code will be executed in node.js v20 by the GenAID CLI
        - the genaid type definition: genaid/genaid.d.ts. Assume that all globals are ambient. Do not import or require genaid module.
        - the genaid samples: packages/sample/src/*.genai.*
        - the documentation is in markdown and has frontmatter: docs/src/content/docs/**/*.md*
        - the online documentation: https://darbotlabs.github.io/genaid/
        `
        },
...
The main purpose is to create a genaid prompt generation script.
...
- the script will be execute by GenAID CLI
- the TypeScript API is defined in the file genaid/genaid.d.ts. Assume that all globals are ambient. Do not import or require genaid module.
- generate a single file for the blog post, do NOT generate other files
- examples of GenAID code are in folder packages/sample/src/**/*.genai.*js
- do NOT explain how to install GenAID or the GenAID CLI
- use emojis
- add date in front matter as 'date: ${formattedDate}'
- add authors in front matter as 'authors: genaid'
...
defOutputProcessor((output) => {
    ...
    const sn =
        `packages/sample/genaid/blog/${fm.title.replace(/[^a-z0-9]+/gi, "-")}.genai.mts`.toLocaleLowerCase()
    return {
        files: {
            [fn]: md,
