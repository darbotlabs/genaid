...
description: Contribution guidelines for building and adding features to GenAID
sidebar:
    order: 100
---

This document provides guidelines for setting up the development environment and building the project.
...
- Go to https://github.com/darbotlabs/genaid
- Click on **Code**
...
GenAID uses the [debug](https://www.npmjs.com/package/debug) library for logging. You can enable logging by setting the `DEBUG` environment variable to `genaid:*`.

```sh
DEBUG=genaid:* yarn genaid <scriptid> ...
```
...
Working on the VSCode involves launching the main project debugger, which opens a second VSCode instance with the GenAID extension.

You can set debug breakpoint anywhere in the GenAID typescript files and they will resolve.

- uninstall the official GenAID extension or it will clash with the locally built one
- open the `Debug` view in Vs Code
- select the **Samples** debugger configuration and click **Run**

Remember that the debugger is only attached to the extension; not the GenAID server.
...
You will find the built package files, `genaid.vsix`,
...
https://github.com/darbotlabs/genaid/actions/workflows/docs.yml .
