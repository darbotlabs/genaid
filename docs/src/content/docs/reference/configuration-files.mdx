...
GenAID supports local and global configuration files to allow reusing common configuration settings and secrets across multiple scripts.

## .env file resolution

GenAID will scan, load the following `.env` files in the following order:

- `envFile` property in the configuration files (see below)
- `GENAID_ENV_FILE` environment variable
- `--env` command line options

```sh
genaid run ... --env ./.env.debug --env ~/.env.dev
```
...
GenAID will scan for the following configuration files
...
The configuration schema is at https://darbotlabs.github.io/genaid/schemas/config.json .
