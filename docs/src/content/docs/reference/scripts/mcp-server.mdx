...
**GenAID implements a server that turns scripts into MCP tools**.

:::tip

GenAID also implements a client for MCP tools which allows you to consume MCP tools in script.
See [MCP tools](/genaid/reference/scripts/mcp-tools) for more details.

:::

## Scripts as MCP Tools

GenAID launches a MCP server that exposes each GenAID script as a MCP tool (not to be confused with `defTool`).
...
The MCP tool parameters is inferred from the [script parameters](/genaid/reference/scripts/parameters) and files automatically.
...
Or any content that was passed in [env.output](/genaid/reference/scripts/output-builder).
...
... // genaid logic with inline prompts
...
GenAID has a built-in [secret scanning feature](/genaid/reference/scripts/secret-scanning)
...
```sh
genaid mcp --startup load-resources
```
...
```sh
npx --yes @modelcontextprotocol/inspector npx --yes genaid mcp
```
...
```json title=".vscode/mcp.json"
{
    "servers": {
        "genaid": {
            "type": "stdio",
            "command": "npx",
            "args": ["-y", "genaid", "mcp", "--cwd", "${workspaceFolder}"],
...
GenAID will do a shallow clone of the repository and run the script from the clone folder.

```sh
npx --yes genaid mcp --remote https://github.com/...
```
