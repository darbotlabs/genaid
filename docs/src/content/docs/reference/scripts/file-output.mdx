...
Reliable file generation, whether new or updates, is one of the most challenging parts of working with LLMs. The GenAID script supports a few approaches and formats to generate files: for small files, regenerating the entire content is typically more efficient.
For large files, generating edits is more efficient.

## How it works

GenAID automatically adds a [system message](/genaid/reference/scripts/system#systemfiles) that teaches the LLM how to format the output files.

Let's start with a script that generates a poem and asks the GenAID to save it to a text file.

```js title="poet.genai.mjs"
$`Generate a 1 sentence poem and save it to a text file.`
```

Since no system prompt is specified, GenAID adds the default set of system prompts, including the [system.files](#system) prompt. This prompt instructs the LLM to generate a file with the output of the script.
The LLM responds with a code section that also mentions a filename. This is the format that GenAID can automatically parse out.
...
```sh
npx genaid run poet --apply-edits
```
...
For large files, GenAID uses a custom `changelog` format that is designed to minimize hallucinations.
...
GenAID will parse this output and generate a file edit similar to a full file update.\
...
In our example, we tell the LLM to produce the poem at `poem.txt` and it also allows GenAID to validate the file location and automatically apply the changes.
...
In the background, GenAID adds a system message that looks like this and tells the LLM where files should be.
...
You can associate a [JSON schema](/genaid/reference/scripts/schemas) with the file output. This schema is used to validate the content of the file before it is written to disk.
...
using [defOutputProcessor](/genaid/reference/scripts/custom-output/).
...
-   [system.files](/genaid/reference/scripts/system#systemfiles), instructs the "full" file format
-   [system.changelog](/genaid/reference/scripts/system#systemchangelog), instructs the "changelog" file format
-   [system.files](/genaid/reference/scripts/system#systemfiles_schema), instructs JSON schema in file generation
