...
sidebar:
  order: 90
description: Explore various fence formats supported by GenAID for optimal
  LLM input text formatting.
keywords: fence format, LLM input, markdown, xml, GenAID
hero:
  image:
...
GenAID supports various types of "fence" formats when rendering [def](/genaid/reference/scripts/context) function, since LLMs may behave differently depending on the format of the input text.
...
GenAID will automatically pick a format based on the model. However, you can override the format at the script level.
