...
GenAID provides APIs to post a message, with file attachments, to a given
...
GenAID uses the Azure authentication client to interact with the Microsoft Graph.
...
To use the Microsoft Teams integration with the [CLI](/genaid/reference/cli),
you need to provide a link url to a Teams channel.

```txt .env
GENAID_TEAMS_CHANNEL_URL=https://teams.microsoft.com/l/...
```
...
For videos, GenAID will split the description into a subject/message
