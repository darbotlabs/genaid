...
GenAID supports transcription and translations from OpenAI like APIs.
...
GenAID will use it as the full path to the ffmpeg executable.
Otherwise, it will attempt to call ffmpeg directly
(so it should be in your PATH).

## model

By default, the API uses the `transcription` [model alias](/genaid/reference/scripts/model-aliases) to transcribe the audio.
...
You can use [Whisper ASR Webservice](/genaid/getting-started/configuration/#whisperasr) to run Whisper locally
...
GenAID renders the segments to [SRT](https://en.wikipedia.org/wiki/SubRip)
