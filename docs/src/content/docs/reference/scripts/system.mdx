...
sidebar:
    order: 10
description: Learn how to utilize system prompts to enhance script execution in GenAID.
...
When unspecified, GenAID inspects the source code of the script
to determine a reasonable set of system prompts ([source code](https://github.com/darbotlabs/genaid/blob/main/packages/core/src/systems.ts)).
...
GenAID comes with a number of system prompt that support features like creating files, extracting diffs or
generating annotations. If unspecified, GenAID looks for specific keywords to activate the various system prompts.
...
Teaches the file format supported by GenAIDs
