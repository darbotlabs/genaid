...
**GenAID implements a client for MCP servers/tools**.

<YouTube id="https://youtu.be/q4Um2Mlvxy8" posterQuality="high" />

:::tip

GenAID also implements a server for MCP which exposes any script as a tool.
See [MCP server](/genaid/reference/scripts/mcp-server) for more details.
...
You can use [defTool](/genaid/reference/scripts/tools) to declare a set of server configurations,
...
GenAID will launch the server and register all the tools listed by the server.
...
This means that if you define servers in an [inline prompt](/genaid/reference/scripts/inline-prompts),
...
GenAID implements various protection mechanisms to mitigate these risks. However, it is important to
...
GenAID supports setting the `signature` of the tools declared by a server. If the tools signature does not match,
GenAID will refuse to load the server (and throw an error).
...
To prevent this, the [secret scanner](/genaid/reference/scripts/secret-scanning)
on all tool outputs.
