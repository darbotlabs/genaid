...
This page describes the way parameter signatures are defined in GenAIDs.
Various entities in GenAID can be parameterized and the `PromptParametersSchema`
...
Internally, GenAID converts a `parameters` object (`PromptParametersSchema`) to a JSON Schema (`JSONSchema`) for various purposes.
...
is used by Visual Studio Code when launching the script, in the [playground](/genaid/reference/playground)
...
- the `parameters` of a [system script](/genaid/reference/scripts/system)
