...
You can also find AI vulnerabilities, such as bias, toxicity, and factuality issues, using the [redteam](/genaid/reference/scripts/redteam) feature.
...
using the JSON schema at [https://darbotlabs.github.io/genaid/schemas/tests.json](https://darbotlabs.github.io/genaid/schemas/tests.json).
...
By default, GenAID extracts the `text` field from the output before sending it to PromptFoo.
...
You can run tests from Visual Studio Code or using the [command line](/genaid/reference/cli).
In both cases, genaid generates a [promptfoo configuration file](https://promptfoo.dev/docs/configuration/guide)
and execute promptfoo on it.

### Visual Studio Code

- Open the script to test
- Right click in the editor and select **Run GenAID Tests** in the context menu
