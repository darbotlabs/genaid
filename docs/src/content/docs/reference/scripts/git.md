...
Since GenAID uses git, it already supports the `.gitignore` instructions. You can also provide additional repository-wide ignore through the `.gitignore.genaid` file at the workspace root. Legacy `.gitignore.genai` is still honored.

```txt title=".gitignore.genaid"
**/genaid.d.ts
```
...
The clones are created under the `.genaid/git/...` directory and are cached based
on the `repository/branch/commit` information.

```js
const clone = await git.shallowClone("darbotlabs/genaid")
```
