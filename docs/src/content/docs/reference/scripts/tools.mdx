---
title: Tools
description: Learn how to define and use tools within GenAID to enhance
...
import weatherScriptSource from "../../../../../../packages/sample/genaid/weather.genai.js?raw"
import mathScriptSource from "../../../../../../packages/sample/genaid/math-agent.genai.mjs?raw"
...
Not all LLM models support tools, in those cases, GenAID also support a fallback mechanism to implement tool call through system prompts (see [Fallback Tools](#fallbacktools)).
...
The parameters are defined using the [parameters schema](/genaid/reference/scripts/parameters).
...
script({
    system: [
        "system.agent_git", // git operations on current repository
        {
            ...
            parameters: { repo: "darbotlabs/genaid" } // but with new parameters
            variant: "genaid" // appended to the identifier to keep tool identifiers unique
        }
    ]
})
```
...
See [Model Context Protocol Tools](/genaid/reference/scripts/mcp-tools) for more information.
...
The tool support is implemented in [system.tool_calls](/genaid/reference/scripts/system#systemtool_calls)
...
GenAID maintains a list of well-known models that do not support
...
To prevent this, you can enable the `detectPromptInjection` option. It will run your [content safety scanner](/genaid/reference/scripts/content-safety) services
...
You can configure GenAID to execute a LLM-as-a-Judge validation of the tool result based on the description or a custom intent.
