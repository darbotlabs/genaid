...
and adding them as images to the prompt. This can be tedious and GenAID provides efficient helpers
...
- specify the transcript computed by the [transcribe](/genaid/reference/scripts/transcription) function. GenAID
...
for videos when using [transcribe](/genaid/reference/scripts/transcription).
...
The [cli](/genaid/reference/cli/video) supports various command to run the video transformations.
