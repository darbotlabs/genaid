...
sidebar:
  order: 0
description: Learn how to use and customize GenAID templates for efficient
  AI prompt expansion.
...
GenAID are JavaScript files named as `*.genai.mjs`, or TypeScript files named as `*.genai.mts`,
...
-   GenAID will detect any file matching `*.genai.mjs`, `*.genai.js`, 
    `*.genai.mts` in your workspace.
-   GenAID files can be placed anywhere in your workspace; 
    but the extension will place them in a `genaid` folder by default.
-   `.genai.mjs` use module JavaScript syntax and support [imports](/genaid/reference/scripts/imports). 
-   `.genai.js` are eval-ed and do not support imports.
-   `.genai.mts` are [TypeScript module files](/genaid/reference/scripts/typescript) and support [imports](/genaid/reference/scripts/imports), 
...
- /genaid
  ...
  - genaid.d.ts  // TypeScript definitions 
  - myscript.genai.mjs // your script!
  - ...

</FileTree>

-   `system.*.genai.mjs` are considered [system prompt templates](/genaid/reference/scripts/system)
