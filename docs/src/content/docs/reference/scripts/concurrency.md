...
When working with GenAID, your program will likely be idle, waiting for tokens to return from the LLM.
...
This feature is leveraged in [inline prompts](/genaid/reference/scripts/inline-prompts) to wait for a LLM result or run multiple queries concurrently.
...
Note that GenAID automatically limits the number of concurrent requests to a single model to prevent this scenario.
