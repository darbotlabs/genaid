...
This technique works reasonably well and GenAID also provides automatic validation "just in case".

You will notice that the schema supported by GenAID is much simpler than the full-blow JSON schema specification. We recommend using simple schemas to avoid confusing the LLM; then port them to your application
...
GenAID supports JSON schemas to define the structure of the data you want to generate.
...
- `responseType: "json"`: The schema is encoded in a system message and validated by GenAID.
- `responseType: "json_object"`: The schema is encoded in the request, using the builtin LLM structured output support.
  It is also validated by GenAID.
...
The [GenAID runtime](/genaid/reference/scripts/runtime) exposes the `z` module.

A [Zod](https://zod.dev/) type can be passed in `defSchema` and it will be automatically converted to JSON schema.
The GenAID also exports the `z` object from Zod for convenience.
...
and the [cli](/genaid/reference/cli) will write the generated TypeScript files
...
GenAID automatically validates the payload against the schema.
...
GenAID will automatically try to repair the data by issues additional messages
