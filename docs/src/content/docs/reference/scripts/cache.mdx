...
hero:
  image:
    alt: A simple 8-bit icon displays a gray folder marked as ".genaid,"
...
```sh "--cache"
npx genaid run ... --cache
```

The cache is stored in the `.genaid/cache/chat.jsonl` file. You can delete this file to clear the cache.
...
The name will be used to create a file in the `.genaid/cache` directory.
...
```sh
npx genaid run .... --cache-name summary
```
