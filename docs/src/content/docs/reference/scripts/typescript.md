...
[TypeScript](https://www.typescriptlang.org/) is a strongly typed programming language that builds on JavaScript, giving you better tooling at any scale. GenAID scripts can be authored in TypeScript.
...
It is possible to [import](/genaid/reference/scripts/imports) TypeScript source file.
...
## Does GenAID type-check prompts?
...
```sh
genaid scripts compile
```

However, at runtime, GenAID converts TypeScript to JavaScript **without type checks** through [tsx](https://tsx.is/usage#no-type-checking).
