...
sidebar:
  order: 20
description: Learn how to enable module imports in GenAID scripts by converting
  them to .mjs format and using static or dynamic imports.
keywords: imports, modules, .mjs, GenAID scripts, JavaScript
hero:
  image:
...
You can import [TypeScript module files](/genaid/reference/scripts/typescript) (`.mts`).
...
The file path is relative to the genaid source file.

```js title="script.genai.mts"
import data from "./data.json" with { type: "json" }

console.log(data.name) // GenAID
```

## Default function export

If you set a function as the default export, GenAID will call it.
