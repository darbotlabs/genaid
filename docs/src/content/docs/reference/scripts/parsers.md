...
The `parsers.YAML` function parses the [YAML format](/genaid/reference/scripts/yaml).
...
## [XML](/genaid/reference/scripts/xml)
...
You can use the `parsers.frontmatter` or [MD](/genaid/reference/scripts/md) to parse out the metadata into an object

```js
const meta = parsers.frontmatter(file)
```

## [CSV](/genaid/reference/scripts/csv)
...
## [PDF](/genaid/reference/scripts/pdf)

The `parsers.PDF` function reads a PDF file and attempts to cleanly convert it into a text format. Read the [/genaid/reference/scripts/pdf](/genaid/reference/scripts/pdf) for more information.

## [DOCX](/genaid/reference/scripts/docx)

The `parsers.DOCX` function reads a .docx file as raw text.

## [INI](/genaid/reference/scripts/ini)
...
## [XLSX](/genaid/reference/scripts/xlsx)
...
[Prompty](/genaid/reference/scripts/prompty) is a markdown-based prompt template format. GenAID provides a parser for prompty templates, with a few additional frontmatter fields
...
Parse output of LLM similar to output of genaid def() function.
...
By default, uses `sha-1`, but `sha-256` can also be used. The hash packing logic may change between versions of genaid.
...
Use the [parse](/genaid/reference/cli/commands#parse) command from the CLI to try out various parsers.

```sh
# convert any known data format to JSON
genaid parse data mydata.csv
```
