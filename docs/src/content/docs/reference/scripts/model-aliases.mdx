...
in a configuration file, through the [cli](/genaid/reference/cli/run) or in the `script` function.

This `.env` file defines a `llama32` alias for the `ollama:llama3.2:1b` model.

```txt title=".env"
GENAID_MODEL_LLAMA32="ollama:llama3.2:1b"
```
...
- [configuration file](/genaid/reference/configuration-files) with the `modelAliases` field
...
- environment variables with keys of the pattern `GENAID_MODEL_ALIAS=...`
- [cli](/genaid/reference/cli/run) with the `--model-alias` flag

```sh
genaid run --model-alias llama32=ollama:llama3.2:1b
```
...
By default, GenAID supports the following model aliases, and various candidates
...
The following aliases are also set so that you can override LLMs used by GenAID itself.

- `agent`: `large`. Model used by the Agent LLM.
- `memory`: `small`. Moel used by the agent short term memory.

The default aliases for a given provider can be loaded using the `provider` option in the [cli](/genaid/reference/cli/run).

```sh
genaid run --provider anthropic
```
