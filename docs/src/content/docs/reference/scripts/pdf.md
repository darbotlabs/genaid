...
description: Learn how to extract text from PDF files for prompt generation
  using GenAID's PDF parsing capabilities.
sidebar:
  order: 9
...
GenAID automatically extracts bitmap images from PDFs and stores them in the data array. You can use these images to generate prompts. The image are encoded as PNG and may be large.
