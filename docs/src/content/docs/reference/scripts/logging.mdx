...
GenAID uses the [debug](https://www.npmjs.com/package/debug) library for logging.
...
```sh "--dbg script"
genaid run poem --dbg script
```

or using the `DEBUG` environment variable.

```sh
DEBUG=script genaid run ...
```

You can specify multiple categories by separating them with a comma.

```sh "--dbg script"
genaid run poem --dbg script file config modelalias
```

or

```sh
DEBUG=script,genaid:* genaid run ...
```
...
Open the GenAID script settings and enable "Diagnostics" (same as setting '\*' as namespace)
...
To turn loggin with the [cli](/genaid/reference/cli),
...
To see log messages, run the script with DEBUG=sample
DEBUG=sample genaid run debug
```
...
console.log("To see log messages, run the script with DEBUG=sample")
...
## GenAID builtin logging

- all internal logging in GenAID is prefixed with `genaid:`.

```sh
DEBUG=genaid:* genaid run ...
```
