...
The `classify` function in GenAID allows you to categorize inputs based on a machine learning model.
It provides a simple interface to leverage the power of LLMs for classification tasks.

## Usage

`classify` is defined in the [GenAID runtime](/genaid/reference/scripts/runtime) and needs to be imported. It takes the text to classify, a set of labels (and options for the LLM)
and returns the label provided by the LLM.

```js
import { classify } from "genaid/runtime"
...
`classify` is provided as part of the runtime (slightly different way to package GenAID functionalities) and needs to be imported using this code...
...
This allows you to select files, images and other GenAID options.
...
The `classify` function uses the `classify` [model alias](/genaid/reference/scripts/model-aliases) by default.
...
The `options` are passed internally to the [inline prompt](/genaid/reference/scripts/inline-prompts) and can be used to modify the behavior of the LLM.

## Assessing classification quality

GenAID returns the [logprob](/genaid/reference/scripts/logprobs) (and entropy) of the classification label. You can use this value to assess the quality of the labelling.
