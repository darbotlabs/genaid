...
description: Learn about the .prompty file format for parameterized prompts and
    its integration with GenAID for AI scripting.
...
GenAID supports running [.prompty](https://prompty.ai/) files as scripts (with some limitations) or importing them in a script. It also provides a parser for those files.
...
There are two ways to leverage prompty files with GenAID:

- run them directly through GenAID
- import them in a script using `importTemplate`

## Running .prompty with GenAID

You can run a `.prompty` file from the [cli](/genaid/reference/cli) or Visual Studio Code as any other `.genai.mjs` script.

GenAID will convert the `.prompty` content as a script and execute it. It supports most of the front matter options but mostly ignores the model configuration section.
...
- model configuration uses GenAID `.env` file (see [configuration](/genaid/getting-started/configuration)).
- images are not yet supported

### Extensions

Extra fields that genaid use:
