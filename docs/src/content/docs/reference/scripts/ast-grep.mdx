...
GenAID provides a wrapper around `ast-grep` to search for patterns in the AST of a script,
...
then copy them back into your GenAID script.
...
[inline prompts](/genaid/reference/scripts/inline-prompts) to perform LLM transformations.
...
GenAID has default mappings from well-known file extensions to languages.
...
GenAID provides a simplified set of interfaces to interact with the `ast-grep` [JavaScript apis](https://ast-grep.github.io/guide/api-usage/js-api.html).
...
You can enable the `genaid:astgrep` namespace to see the queries and results in the logs.

```sh
DEBUG=genaid:astgrep ...
```
