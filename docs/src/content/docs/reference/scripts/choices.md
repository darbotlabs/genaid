...
- For some models, GenAID does not have a token encoder so it won't be able to compute the logit bias for the choices
...
For models where GenAID does not have a token encoder, you can provide the pre-encoded tokens.
...
Internally, GenAID tokenizes the word and build the [logit_bias](https://help.openai.com/en/articles/5247780-using-logit-bias-to-alter-token-probability-with-the-openai-api) for each token.
...
You can enable [logprobs](/genaid/reference/scripts/logprobs) to visualize the confidence of the tokens generated by the model.
