...
GenAID provides access to the file system of workspace and to the selected files in the user interface.
...
You can pass `env.files` directly in the [def](/genaid/reference/scripts/context)
...
## `.gitignore` and `.gitignore.genaid`

By default, the `.gitignore` (workspace level) and `.gitignore.genaid` (project level) files are respected when selecting files. Legacy `.gitignore.genai` is still honored.
...
```sh
genaid run --ignore-git-ignore
```

`.gitignore.genaid` is an extra file that is used to filter files in the project. It is useful when you want to exclude files from the project that are not relevant for the script beyond the `.gitignore` file.

## file output

Use [defFileOutput](/genaid/reference/scripts/file-output) to specify allowed file output paths and the description
...
You can provide a [JSON schema](/genaid/reference/scripts/schemas) to validate the parsed data.
