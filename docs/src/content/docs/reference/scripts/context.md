...
description: Detailed documentation on the script execution context and
  environment variables in GenAID.
keywords: script execution, env object, GenAID context, def function, env files
hero:
  image:
...
`env` is populated automatically by the GenAID runtime.
...
- [CLI](/genaid/reference/cli) files arguments.
...
Read more about [variables](/genaid/reference/scripts/variables).
...
or if the model support XML tags (see [fence formats](/genaid/reference/scripts/fence-formats)):
...
You can specify the language of the text contained in `def`. This can help GenAID optimize the rendering of the text.
...
The `def` function treats data files such as [CSV](/genaid/reference/scripts/csv) and [XLSX](/genaid/reference/scripts/xlsx) specially. It will automatically convert the data into a
...
Read more about [prompt caching](/genaid/reference/scripts/prompt-caching).

### Safety: Prompt Injection detection

You can schedule a check for prompt injection/jai break with your configured [content safety](/genaid/reference/scripts/content-safety) provider.
