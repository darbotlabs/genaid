...
import sourceMulti from "../../../../../../packages/sample/genaid/agent_git_other.genai.mjs?raw"

GenAID defines an **agent** as a [tool](/genaid/reference/scripts/tools) that
runs an [inline prompt](/genaid/reference/scripts/inline-prompts) to accomplish a task. The agent's LLM is typically augmented with
...
**GenAID does _not_ implement any agentic workflow or decision.**
It relies entirely on [tools](/genaid/reference/scripts/tools) support built into the LLMs.
...
This full source of this agent is defined in the [system.agent_github](/genaid/reference/scripts/system/#systemagent_github) system prompt.

## Logging

Each agent uses a `agent:<name>` [logging](/genaid/reference/scripts/logging) namespace
