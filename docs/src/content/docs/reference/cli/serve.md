...
```bash
npx genaid serve --port 8004
```

## API key

The API key is used to authenticate the requests to the server.
You can specify an API key by setting the `--api-key` flag or the `GENAID_API_KEY` environment variable.

```bash
npx genaid serve --api-key my-api-key
```

or

```txt title=".env"
GENAID_API_KEY=my-api-key
```
...
You can enable [Cross Origin Shared Resource](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) by setting the `--cors` flag or setting the `GENAID_CORS_ORIGIN` environment variable.

```bash
npx genaid serve --cors contoso.com
```
...
```sh
docker run --name genaid --rm -it --expose 8003 -p 8003:8003 -v ${PWD}:/workspace -w /workspace node:alpine npx --yes genaid serve --network
```
