...
```bash
npx genaid convert <script> "<files...>"
```
...
npx genaid convert <script> <files> --excluded-files <excluded-files...>
```
...
npx genaid convert <script> <files> --exclude-git-ignore
```
...
```sh "--suffix .genai.txt"
npx genaid convert <script> <files> --suffix .genai.txt
```

GenAID will "unfence" output in the markdown that match the suffix (after `.genai`) automatically. So if the LLM generates
...
This flag override `suffix` and tells GenAID to rewrite the original file with the converted content.

```sh "--rewrite"
npx genaid convert <script> <files> --rewrite
```
...
```sh '--cancel-word "<NO>"'
npx genaid convert <script> <files> --cancel-word "<NO>"
```

## Read more

The full list of options is available in the [CLI reference](/genaid/reference/cli/commands#convert).
