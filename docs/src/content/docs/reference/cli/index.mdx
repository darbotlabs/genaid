...
sidebar:
  order: 1
description: Comprehensive guide to using the GenAID CLI for automating
  tasks with AI scripts in Node.js environments.
keywords: GenAID CLI, Node.js automation, AI scripting, command line
...
The GenAID CLI **`genaid`** runs GenAID scripts
outside of Visual Studio and in your [automation](/genaid/getting-started/automating-scripts).

<GenAIDCli args="..." />

## Prerequisites

The CLI is a Node.JS package hosted on [npm](https://www.npmjs.com/package/genaid).
...
<PackageManagers pkg="genaid" dev frame="none" />
...
{
    "scripts": {
        "postinstall": "genaid scripts fix",
        "postupdate": "genaid scripts fix",
        "genaid": "genaid"
    }
}
```

The `genaid` is also a shorthand script that makes it easier to invoke the CLI
using `npm run`:
...
The CLI will load the [secrets](/genaid/getting-started/configuration) from the environment variables or a `./.env` file.

You can override the default `.env` file name by adding the `--env .env.local` file,
over even import both.

```sh
npx genaid run <script> --env .env .env.local
```

## Create a new script

Creates a new script file in the `genaid` folder.

```sh
npx genaid scripts create <name>
```
...
```sh
npx genaid scripts compile
```

## Run a script

[Run a script](/genaid/reference/cli/run) on file
...
```sh
npx genaid scripts model [script]
```
...
The CLI can be imported and [used as an API in your Node.JS application](/genaid/reference/api).
...
```sh
genaid run <script> [files...] --vars key1=value1 key2=value2
```
