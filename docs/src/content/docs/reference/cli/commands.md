...
```
Usage: genaid configure [options]
...
```
Usage: genaid run [options] <script> [files...]

Runs a GenAID against files.
...
Options:
  ...
  --vars <namevalue...>                      variables, as name=value, stored in env.vars. Use environment variables GENAID_VAR_name=value to pass variable through the environment
...
```
Usage: genaid runs [options] [command]
...
```
Usage: genaid runs list [options] [script]
...
```
Usage: genaid test|eval [options] [command]
...
```
Usage: genaid test run [options] [script...]
...
```
Usage: genaid test list [options]
...
```
Usage: genaid test view [options]
...
```
Usage: genaid convert [options] <script> [files...]

Converts file through a GenAID. Each file is processed separately through
the GenAID and the LLM output is saved to a <filename>.genai.md (or custom
suffix).
...
```
Usage: genaid scripts|script [options] [command]
...
```
Usage: genaid scripts list [options] [script...]
...
```
Usage: genaid scripts create [options] [name]
...
```
Usage: genaid scripts fix [options]
...
Options:
  -gci, --github-copilot-instructions  Write GitHub Copilot custom instructions
                                       for better GenAID code generation
...
```
Usage: genaid scripts compile [options] [folders...]
...
```
Usage: genaid scripts model [options] [script]
...
```
Usage: genaid scripts help|info [options] <script>
...
```
Usage: genaid cache [options] [command]
...
```
Usage: genaid cache clear [options] [name]
...
```
Usage: genaid video [options] [command]
...
```
Usage: genaid video probe [options] <file>
...
```
Usage: genaid video extract-audio [options] <file>
...
```
Usage: genaid video extract-frames [options] <file>
...
```
Usage: genaid retrieval|retreival [options] [command]
...
```
Usage: genaid retrieval index [options] <name> <files...>
...
```
Usage: genaid retrieval vector|search [options] <query> [files...]
...
```
Usage: genaid retrieval fuzz [options] <query> [files...]
...
```
Usage: genaid serve [options]

Start a GenAID local web server
...
```
Usage: genaid parse|parsers [options] [command] <file...>
...
Commands:
  ...
  prompty [options] <file...>   Converts .prompty files to genaid
...
```
Usage: genaid parse data [options] <file>
...
```
Usage: genaid parse fence [options] <language> <file>
...
```
Usage: genaid parse pdf [options] <file>
...
```
Usage: genaid parse docx [options] <file>
...
```
Usage: genaid parse html [options] <file_or_url>
...
```
Usage: genaid parse code [options] <file> [query]
...
```
Usage: genaid parse tokens [options] <files...>
...
```
Usage: genaid parse tokenize [options] <file>
...
```
Usage: genaid parse jsonl2json [options]
...
```
Usage: genaid parse prompty [options] <file...>

Converts .prompty files to genaid
...
```
Usage: genaid parse jinja2 [options] <file>
...
```
Usage: genaid parse secrets [options] <file...>
...
```
Usage: genaid parse markdown [options] <file>
...
```
Usage: genaid info [options] [command]
...
```
Usage: genaid info help [options]
...
```
Usage: genaid info system [options]
...
```
Usage: genaid info env [options] [provider]
...
```
Usage: genaid models [options] [command]
...
```
Usage: genaid models list [options] [provider]
...
```
Usage: genaid models alias [options]
