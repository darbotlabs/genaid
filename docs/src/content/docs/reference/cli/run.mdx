---
title: Run
description: Learn how to execute GenAID scripts on files with streaming output
...
```sh
npx genaid run code-annotator "src/*.ts"
```

If multiple files are specified, all files are included in `env.files`.

```sh
npx genaid run <script> "src/*.bicep" "src/*.ts"
```
...
```bash
npx genaid run <script> "**/*.md" "**/*.ts"
```

### Resource resolutions

GenAID will automatically handle and resolve specific URI patterns.
...
```bash
cat README.md | genaid run summarize > summary.md
```
...
npx genaid run <script> <files> --excluded-files <excluded-files...>
```
...
npx genaid run <script> <files> --exclude-git-ignore
```
...
```sh "--out tmp"
npx genaid run <script> <files> --out out/res.json
```

If `file` does not end with `.json`, the path is treated as a directory path.

```sh "--out tmp"
npx genaid run <script> <files> --out tmp
```
...
```sh wrap
npx genaid run <script> <files> --out-trace &lt;file&gt;
```

In a GitHub Actions workflow, you can use this feature to save the trace as a step summary (`GITHUB_STEP_SUMMARY`):

```yaml title=".github/workflows/genaid.yml" wrap
- name: Run GenAID tool on spec
  run: |
      genaid run <script> <files> --out-trace $GITHUB_STEP_SUMMARY
```
...
```yaml title="genaid.pipeline.yml" "##vso[task.uploadsummary]" wrap
- script: npx --yes genaid run poem --out-trace $(System.DefaultWorkingDirectory)/trace.md
  displayName: "Run GenAID tool"
...
```sh wrap
npx genaid run <script> <files> --out-annotations diags.csv
```

Use JSON lines (`.jsonl`) to aggregate annotations from multiple runs in a single file.

```sh wrap
npx genaid run <script> <files> --out-annotations diags.jsonl
```
...
```sh
npx genaid run <script> <files> --out-data data.jsonl
```
...
```sh
npx genaid run <script> <files> --out-changelogs changelogs.txt
```
...
```yaml
    - run: npx --yes genaid run ... -prc --out-trace $GITHUB_STEP_SUMMARY
      env:
...
Upserts a comment on the pull request/issue with the LLM output
...
Create pull request review comments from each [annotations](/genaid/reference/scripts/annotations)
.
...
The full list of options is available in the [CLI reference](/genaid/reference/cli/commands#run).
