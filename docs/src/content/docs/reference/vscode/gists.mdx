---
title: GitHub Gists
description: Use gists to share GenAID code snippets.
sidebar:
  order: 20
hero:
  image:
...
## Running GenAID from Gists

GenAID supports the following URL formats to run scripts directly from a gist.

- `gist://<gist id>/<file name>`
- `vscode://vsls-contrib.gistfs/open?gist=<gist id>&file=<file>`

```sh
genaid run gist://8f7db2674f7b0eaaf563eae28253c2b0/poem.genai.mts
```

The gist file is cached locally in `.genaid/resources` then executed. If available,
it uses the GitHub login information to access private gists.

:::caution

GenAID are JavaScript files so make sure you run gists you trust.
...
You can open a file in a Gist and run it using the `genaid` command.

### Type Checking

To get type checking working, we need to upload the `genaid.d.ts` to the gist and setup a reference to it
by adding this comment **at the top of the file**:

```js
/// <reference path="./genaid.d.ts" />
```

This can be done automatically:

- right click on the Gist GenAID file
- select `GenAID: Fix Type Definitions`
- You might be prompted to allow GenAID to use your GitHub account. GenAID will request a token with `gist` scope to upload the missing files.

In order to load the GenAID types, you'll need to "nudge" the TypeScript compiler:

- open the `genaid.d.ts` file from the GistPad tree (this loads the types in memory)
- open your GenAID file in the GistPad tree and it should have type checking!
