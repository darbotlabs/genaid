...
import scriptSource from "../../../../../../packages/sample/genaid/samples/copilotchat.genai.mjs?raw"
import src from "../../../../assets/chat-participant.png"
import alt from "../../../../assets/chat-participant.png.txt?raw"
import { Steps } from "@astrojs/starlight/components"

GenAID integrates with [GitHub Copilot Chat](https://marketplace.visualstudio.com/items?itemName=GitHub.copilot-chat)
by providing a **chat participant** that allows you to run scripts in the context of a chat conversation,
and a **custom prompt** to generate GenAID more efficiently with Copilot Chat.

## `@genaid` chat participant

The `@genaid` [chat participant](https://code.visualstudio.com/api/extension-guides/chat#parts-of-the-chat-user-experience) lets your run scripts without the context
...
```sh
@genaid /run summarize
```

If you omit the `/run` command, GenAID will look for a script named `copilotchat`. If it finds one, it will run it.
Otherwise, it will ask you to pick a script from the list of available scripts.

```sh
@genaid add comments to the current editor
```

### Choosing the model

If your script does not specify a model, GenAID will prompt you to choose a model.
...
When GenAID prompts you to choose a model, it will store your choices in the workspace settings
under the

```json file=".vscode/settings.json"
{
    "genaid.languageChatModels": {
...
the [cli](/genaid/reference/cli) or [playground](/genaid/reference/playground) interfaces.
...
When GenAID tries to access a model that is not available, it will notify you but it does not have
...
You can use the `@genaid` chat to weave the execution of a script into an existing conversation
...
- `@genaid /run tool` will run the `tool` script and place the results back into the chat history.
...
<Code
    ...
    wrap={true}
    lang="ts"
    title="genaid/copilotchat.genai.mts"
/>
...
## `genaid` custom instructions <a href="" id="genaid-custom-prompt" />

GenAID will automatically save an instructions.md file in the `.genaid/instructions` folder
when you run a script. This file contains the instructions used to generate the script.

### Augmented chat sessions

This is how you start chat sessions using the `genaid` prompt.
...
then select the **genaid.instructions.md** prompt.

</li>

<li>

Include instructions to write a script or answer a question about GenAID,
...
Since the prompt injects the entire documentation of GenAID (700+kb at this time of writing),
