...
sidebar:
    order: 3
description: Learn how to run GenAIDs in Visual Studio Code using the
    GenAID extension.
keywords: GenAID, Visual Studio Code, run scripts, command palette, context menu
hero:
    image:
        alt:
...
The GenAID extension for Visual Studio Code provides a convenient way to run scripts directly from the editor.
...
- open a GenAID file in the editor
- right-click in the editor and select **Run GenAID\*** form the content menu
- or click on on the **Run GenAID** icon in the top right corner of the editor
...
- right-click and select **Run GenAID** from the context menu

### From an editor

- open an file in the editor (not a GenAID file)
- right-click and select **Run GenAID** from the context menu

## Using the selected text in your script

Whenever you launch a script, GenAID will grab the selected text in the active text editor
...
GenAID tries to respect the **top-level `.gitignore` rules in the project workspace** (it ignores nested .gitignore files).
This means that if you have a `.gitignore` file in your project, GenAID will ignore any files or folders that are ignored by Git.

There are exceptions to this rule:

- if you run **Run GenAID** on individual files, those files are not filtered by `.gitignore` since you explicitly chose them. Folders are always filtered.
- if you specify `---ignore-git-ignore` in the command line, GenAID will ignore the `.gitignore` file and run the script on all files and folders.
