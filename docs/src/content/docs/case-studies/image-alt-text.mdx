...
import scriptSrc from "../../../../genaid/image-alt-text.genai.mjs?raw"
...
and we use [defImage](/genaid/reference/scripts/images) to add it to the prompt context.
...
finally, we use [defFileOutput](/genaid/reference/scripts/file-output) to define
a file output route.
...
The GenAID documentation uses Astro, which allows to author pages in [MDX](https://docs.astro.build/en/guides/markdown-content/).
...
Using the [run](/genaid/reference/cli/run) command, we can apply the script to each image in the docs.

```sh
for file in assets/**.png; do
  npx --yes genaid run image-alt-text "$file"
```
