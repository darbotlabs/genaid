...
GenAID allowed to develop and automate a script that create high-quality LLM-based translations
...
Note that we use `env.vargs.lang` [variable](/genaid/reference/scripts/variables) which allows to modify this value through the command line.

```js
const langName = env.vars.lang || "French"
```

Using the genaid CLI, we can run the script for each desired language in a GitHub Action.

```sh
npx genaid run translate ... --vars lang=German
```
...
for(const lang of langs) {
    ...
    await $`genaid run translate ... --vars lang=${lang} ... --apply-edits`
