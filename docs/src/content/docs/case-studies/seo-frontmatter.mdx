...
sidebar:
  order: 2
description: Learn how to automate the creation of SEO-optimized front matter for your markdown documents with GenAID.
keywords: SEO automation, front matter generation, markdown SEO, content optimization, metadata management
---

import { Code, Steps } from '@astrojs/starlight/components';
import importedCode from "../../../../genaid/frontmatter.genai.mjs?raw"

Generating and maintaining good SEO front matter fields can be a tedious task. 
GenAID can help you automate this process.
...
the [CLI](/genaid/reference/cli). The CLI has a **--apply-edits** flag to apply the changes to the file.

```sh
for file in src/**/*.md; do
  genaid run frontmatter "$file" --apply-edits
```
