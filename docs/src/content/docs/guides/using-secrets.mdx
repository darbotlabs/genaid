...
import tavilyCode from "../../../../../packages/sample/genaid/tavily.mjs?raw"
import scriptCode from "../../../../../packages/sample/genaid/document-augmentor.genai.mjs?raw"


This guide shows how to use TypeScript, a 3rd party search service, and [secrets](/genaid/reference/scripts/secrets) to create
...
We define a function `tavilySearch` in [TypeScript](/genaid/reference/scripts/typescript) that wraps the `fetch` call and we add type annotations to provide 
...
The `tavilySearch` function is imported using a dynamic [import](/genaid/reference/scripts/imports).
