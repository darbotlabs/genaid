...
import src from "../../../../../packages/sample/genaid/video-alt-text.genai.mjs?raw"

GenAID supports [speech transcription](/genaid/reference/scripts/transcription)
and [video frame extraction](/genaid/reference/scripts/videos) which can be combined to analyze videos.
...
The next step is to use the transcript to screenshot the video stream. GenAID uses [ffmpeg](https://ffmpeg.org/) to render the frames
...
```sh
genaid run video-alt-text path_to_video.mp4
```
