...
description: Learn how to run a GenAID over many documents
...
Suppose I have a directory with multiple `.pdf` (or other) files and I want to run a GenAID over all of them.
...
<ol>
<li>
Use the `> GenAID: Create new script...` command in the command palette to create a new script.
</li><li>
...
Select **Run GenAID**. Select the script `gen-tweet` you just wrote.
</li><li>
Assuming we give the GenAID a paper describing GenAID, the Output will be displayed in a new document tab.

```plaintext wrap
Discover GenAID: a revolutionary scripting language integrating AI to automate complex tasks, making coding accessible to all! #AI #CodingFuture
```
...
<ol>
<li>
We can run the script from the [command line](/genaid/reference/cli/):

```sh wrap
npx genaid run gen-tweet example1.pdf
```
...
for file in *.pdf; do
  ...
  if [ ! -f "$newfile" ]; then # skip if already exists
    npx genaid run gen-tweet $file > $newfile
  fi
done
```
...
Get-ChildItem -Filter *.pdf | ForEach-Object {
  ...
  if (-not (Test-Path $newName)) {
    npx genaid run gen-tweet $_.FullName | Set-Content "$newName"
  }
}
```
...
for (const file of files) {
    ...
    if (!(await fs.exists(out)))
        ...
        await $`genaid run gen-tweet ${file} > ${out}`
}
```
