...
To help with this, we've crafted a [script tailored to automate generating Git commit messages](https://github.com/darbotlabs/genaid/blob/main/packages/sample/genaid/samples/gcm.genai.mts),
ensuring they are meaningful and save you time.

The script acts as a regular node.js automation script and uses [runPrompt](/genaid/reference/scripts/inline-prompts)
to issue calls to the LLM and ask the user to confirm the generated text.

:::tip

You can checkout the [Git Commit Message](/genaid/samples/gcm) sample for a fully-fledge,
ready to use script.
...
You can run this script using the [CLI](/genaid/reference/cli).
...
of genaid in the `.husky/commit-msg` file.
