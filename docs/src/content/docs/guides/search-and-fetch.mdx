...
description: Learn how to leverage web search and fetching pages in GenAID
...
Suppose we want to plan a weekend trip using a GenAID that
will help us plan by using web search to learn about things to do and the expected weather.

:::note

You will need a [Bing Web Search API key](/genaid/reference/scripts/web-search) to use `webSearch`.

:::

<Steps>

1. Use the `> GenAID: Create new script...` command in the command palette to create a new script.
2. Start the script by defining the model and title:
    ...
    ```
3. Use the [`webSearch`](/genaid/reference/scripts/web-search/) function to 
search for information about the destination. 
    ...
    ```
4. `webSearch` returns a list of URLs.  Use [`fetchText`](/genaid/reference/scripts/fetch/)
...
5. `host.fetchText` returns a lot of formatting HTML tags, etc.
    Use [`runPrompt`](/genaid/reference/scripts/inline-prompts/)
