...
import source from "../../../../../packages/sample/genaid/pdfocr.genai.mts?raw"
...
-   one can read the text using [Mozilla's pdfjs](https://mozilla.github.io/pdf.js/) (GenAID uses that), which may give some results but the text might be garbled or not in the correct order. And tables are a challenge. And this won't work for PDFs that are images only.
-   another technique would be to apply OCR algorithm on segments of the image to "read" the rendered text.

In this guide, we will build a GenAID that uses a LLM with vision support to extract text and images from a PDF, converting each page into markdown.
...
This prompt instructs GenAID to convert the page image into markdown. It highlights the use of `pdfjs-dist` for text extraction and instructs how to handle text, tables, and images.
...
To run this script using the GenAID CLI, navigate to your terminal and execute:

```bash
genaid run pdfocr <mypdf.pdf>
```

For more details on installing and setting up the GenAID CLI, refer to the [official documentation](https://darbotlabs.github.io/genaid/getting-started/installation).
