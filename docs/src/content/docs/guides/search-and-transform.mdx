...
import source from "../../../../../packages/sample/genaid/samples/st.genai.mjs?raw"
...
The search step is done with the [workspace.grep](/genaid/reference/scripts/files)
...
and pre-compute the LLM transformation of each match using an [inline prompt](/genaid/reference/scripts/inline-prompts).
...
```sh wrap
genaid st --vars 'pattern=host\.exec\s*\([^,]+,\s*\[[^\]]+\]\s*\)' 'transform=Convert the call to a single string command shell in TypeScript'
```
