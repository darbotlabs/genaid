...
keywords: pull request automation, code review, GitHub Actions, GenAID reviewer, PR feedback
---
import { Code } from '@astrojs/starlight/components';
import importedCode from "../../../../../packages/sample/genaid/issue-reviewer.genai.mjs?raw"

This guide shows how to automate reviewing issues 
with a GenAID that provides feedback and code analysis in GitHub Actions.
...
The `github.getIssue` assumes that GenAID is running in a GitHub Action, 
...
The `-prc` flag stands for [--pull-request-comment](/genaid/reference/cli/run#pull-requests)
...
...
    - run: npx --yes genaid run issue-reviewer -prc --out-trace $GITHUB_STEP_SUMMARY
      env:
        GITHUB_ISSUE: ${{ github.event.issue.number }}
