...
import code from "../../../../../packages/sample/genaid/lint.genai.mjs?raw"
...
The script also uses the built-in support for errors and warnings in GenAID to surface the issues found in the IDE automatically.
...
- **`script({...})`**: This function initializes a GenAID script.
...
To execute this script, you can run it from Visual Studio Code or use the GenAID CLI. For detailed instructions on installation, refer to the [online documentation](https://darbotlabs.github.io/genaid/getting-started).

```bash
genaid run lint <file1> <file2> ...
```
...
The full source code is available at [https://github.com/darbotlabs/genaid/blob/main/packages/sample/genaid/lint.genai.mts](https://github.com/darbotlabs/genaid/blob/main/packages/sample/genaid/lint.genai.mts).
