---
title: Awesome Scripts
description: A curated list of awesome scripts
sidebar:
    order: 0
tags:
    - awesome
    - scripts
    - tools
    - utilities
    - scripts
    - genaid
    - automation
---

import { LinkCard } from "@astrojs/starlight/components"

These are a few scripts written by the community that are useful for various tasks. If you have a script that you think should be included, please submit a pull request to add it to this list.

<LinkCard
    title="Grumpy Dev"
    description="Let the grumpy senior dev review your code with this MCP server"
    href="https://github.com/sinedied/grumpydev-mcp/blob/main/genaid/review-code.genai.js"
/>

- [Submit your script](https://github.com/darbotlabs/genaid/edit/dev/docs/src/content/docs/samples/awesome.mdx).
