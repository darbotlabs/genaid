...
import source from "../../../../../packages/sample/genaid/samples/cmt.genai.mts?raw"
...
We process each file separately to avoid overwhelming the token context and to keep the AI focused. We can use [inline prompts](/genaid/reference/scripts/inline-prompts) to make inner queries.
...
Within the `addComments` function, we prompt GenAID to add comments.
...
To run this script, you'll first need to install the GenAID CLI. [Follow the installation guide here](https://darbotlabs.github.io/genaid/getting-started/installation).
...
```sh
genaid run cmt --vars "build=npm run build" "format=npm run format"
```

## Full source ([GitHub](https://github.com/darbotlabs/genaid/blob/main/packages/sample/genaid/samples/cmt.genai.mts))
