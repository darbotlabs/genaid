...
import source from "../../../../../packages/sample/genaid/samples/iat.genai.mts?raw"
...
This script is composed of TypeScript code, designed to run with the GenAID CLI. Let's break it down:
...
The script uses [workspace.grep](/genaid/reference/scripts/files#grep) to find all occurrences of the regex pattern in the specified documents.

## Generating alt text

For each image URL found, we generate alt text using an [inline prompt](/genaid/reference/scripts/inline-prompts)
and [defImages](/genaid/reference/scripts/images).
...
To run this script, you'll need the GenAID CLI. If you haven't installed it yet, check out the [installation guide](https://darbotlabs.github.io/genaid/getting-started/installation).
...
## Full source ([GitHub](https://github.com/darbotlabs/genaid/blob/main/packages/sample/genaid/samples/iat.genai.mts))
