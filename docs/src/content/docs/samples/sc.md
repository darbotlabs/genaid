...
excerpt: >-
  ...
  precision. This method leverages `GenAID` and GitHub Models to correct
...
- Add the following script to your repository as `sc.genai.mts` in the `genaid` folder.

```ts title="genaid/sc.genai.mts" wrap
script({
    title: "Spell checker",
...
```yaml title=".github/workflows/genaid-sc.yml" wrap
name: genaid sc
on:
    push:
        branches-ignore:
        ...
        paths:
            ...
            - '**/*.mdx'
concurrency:
    group: genaid-sc-{{ github.workflow }}-${{ github.ref }}
...
jobs:
    review:
        ...
        steps:
            ...
            - name: genaid sc
              run: npx --yes genaid run sc --vars base="${{ github.event.before }}" --out-trace $GITHUB_STEP_SUMMARY
              env:
            ...
            - name: Commit and push changes
              run: |
                  ...
                  if git diff --cached --quiet; then
                    ...
                    git commit -m "fix: spellcheck markdown files [genaid]"
