...
import source from "../../../../../packages/sample/genaid/samples/gcm.genai.mts?raw"
...
using an [inline prompt](/genaid/reference/scripts/inline-prompts),
...
First, we define the `script` function, which sets up our GenAID script by providing a title and a description, and specifying the model we'll be using:
...
If there is nothing staged, GenAID kindly offers to stage all changes for you:
...
Here comes the interesting part. We enter a loop where GenAID will generate a commit message for you based on the diff. If you're not satisfied with the message, you can choose to edit it, accept it, or regenerate it:
...
If you choose to commit, GenAID runs the `git commit` command with your message, and if you're feeling super confident, it can even push the changes to your repository right after:
...
## Running the Script with GenAID CLI

Use the [cli](/genaid/reference/cli) to run the script:

```shell
npx genaid run gcm
```

## Full source ([GitHub](https://github.com/darbotlabs/genaid/blob/main/packages/sample/genaid/samples/gcm.genai.mts))
...
or validating the message with a [content safety service](/genaid/reference/scripts/content-safety).
