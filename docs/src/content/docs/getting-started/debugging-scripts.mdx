...
sidebar:
  order: 4.5
description: Learn how to debug GenAID files using Visual Studio Code
...
The GenAID script files are executable JavaScript and can be debugged
...
GenAID also provides [builtin lightweight logging](/genaid/reference/scripts/logging) to help you
...
- Select the GenAID from the picker.
...
The debugger will launch the [cli](/genaid/reference/cli) and run the script in debug mode.
...
Keep iterating the script or [add tests](/genaid/getting-started/testing-scripts).
