...
sidebar:
  order: 1
description: Learn how to install GenAID as a Visual Studio Code extension
  or use it via command line for seamless integration into your development
  workflow.
keywords: Visual Studio Code, GenAID extension, command line tool, manual
...
import vscodeViewSrc from "../../../assets/vscode-genaid-view.png"
import vscodeViewAlt from "../../../assets/vscode-genaid-view.png.txt?raw"

GenAID is available as a [command line](#command-line) or a [Visual Studio Code Extension](#visual-studio-code-extension).

## Node.JS

GenAID requires [Node.JS](https://nodejs.org/) to run.
...
The [Visual Studio Code Marketplace](https://marketplace.visualstudio.com/items?itemName=genaid.genaid-vscode)
contains the latest stable release of the [extension](https://marketplace.visualstudio.com/items?itemName=genaid.genaid-vscode).
...
Search **genaid** and click **Install**.
...
To install a specific version of the extension, we recommend storing the `genaid.vsix`
in your repository and using the manual installation steps.

### Default Profile for Terminal

GenAID launches a node server in the default terminal. If the default terminal is not configured
...
Open the [latest release](https://github.com/darbotlabs/genaid/releases/latest/) on GitHub

</li><li>

Download the `genaid.vsix` into your project root folder

<FileTree>

- ...
- .genaid/ folder created by the extension to store supporting files
...
The [genaid](/genaid/reference/cli/) command line tool lets you run your GenAID
from any terminal.

```sh
npx genaid run my-script some/path/*.pdf
```
...
**npx can be slow**. Learn how to install locally in the [cli reference](/genaid/reference/cli/).

:::

## DevContainer

You can add this file in your project to use GenAID in a
...
```json title=".devcontainer/devcontainer.json"
{
    ...
    "customizations": {
        "vscode": {
            "extensions": ["genaid.genaid-vscode"]
        }
    }
}
```
...
Let's configure the [LLM connection information](/genaid/getting-started/configuration)
