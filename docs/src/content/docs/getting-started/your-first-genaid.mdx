---
title: Your first GenAID script
sidebar:
    order: 3
description: Learn how to create and execute your initial GenAID script to
    automate interactions with language models.
keywords: GenAID script creation, language model automation, script execution,
    LLM prompt generation, genaid setup
genaid:
...
GenAID scripts use stylized JavaScript with minimal syntax.
They are stored as JavaScript files (`genaid/*.genai.mjs`) or TypeScript files (`genaid/*.genai.mts`) in your project.
The execution of a GenAID script creates the prompt that will be sent to the LLM.
...
The resulting file will be placed in the `genaid` folder in your project.

<FileTree>

- …
- genaid scripts are created here by default
    - genaid.d.ts (TypeScript type definitions)
...
Watch this video to learn how to turn super charge the knowledge of GenAID in GitHub Copilot Chat.
...
The execution of the GenAID generates a prompt (and more)
...
GenAID exposes the context through the `env` variable. The context is implicitly defined by the location you start executing the script.
...
- you can run the script using the [command line interface](/genaid/reference/cli) and specify content of `env.files` in the CLI arguments.
...
- Follow the [Prompt As Code guide](/genaid/guides/prompt-as-code) to dive deeper in programmatically generating prompts
- [Run your script](/genaid/getting-started/running-scripts) from Visual Studio Code.
