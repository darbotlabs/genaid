...
sidebar:
    order: 5
description: Learn how to automate your scripts using the GenAID CLI for
...
import prDescribeSrc from "../../../../../packages/sample/genaid/pr-describe.genai.mjs?raw"

Once you have a script that you are happy with, you can automate it using the [command line interface](/genaid/reference/cli).

## Running a script using the CLI

The basic usage of the CLI is to [run](/genaid/reference/cli/run/) a script with a tool name and a list of files.

```sh wrap
npx --yes genaid run <script_id> <...files>
```
...
from the [genaid npm package](https://www.npmjs.com/package/genaid) (either one installed locally, or fetched remotely).
...
```sh wrap "--apply-edits"
npx --yes genaid run <script> <...files> --apply-edits
```
...
branch and a pull request. You can also use a [container](/genaid/reference/scripts/container)
to run the script in a sandboxed environment.

Refer to [Security and Trust](/genaid/reference/security-and-trust) for more discussion.
...
This section explains how to integrate your GenAID in GitHub Actions workflows and pull requests.
...
permissions:
    models: read
...
- run: npx --yes genaid run <script> <...files> --provider github
  env:
...
on your repository or organization so that GenAID can connect to your LLM.

The secrets and variables should match the `.env` file in your local environment.

### Running a script

Use the [cli](/genaid/reference/cli/run/) to run the script in a GitHub Action.
...
```yaml
- run: npx --yes genaid run <script> <...files> --out results
  env:
      # variables
...
(see example in GitHub Actions summary).

```yaml "--out-trace $GITHUB_STEP_SUMMARY"
- run: npx --yes genaid run ... --out-trace $GITHUB_STEP_SUMMARY
```
...
Update each step that invokes the [cli](/genaid/reference/cli) to include the `AZURE_OPENAI_API_ENDPOINT` variable.

```yaml
- name: run GenAID script
...
The `--pull-request-description` inserts the LLM output into the pull request section.
...
- run: npx --yes genaid run --pull-request-description
```
...
The comment is enclosed in two XML comments (`<genaid begin [id]>`, `<genaid end [id]>`)
...
The `--pull-request-comment` adds the LLM output as a comment to the pull request conversation.
...
- run: npx --yes genaid run --pull-request-comment
  env: ...
```

### Review comments

Use the `--pull-request-reviews` to convert [annotations](/genaid/reference/scripts/annotations) as review comments
**to the last commit** on the pull request.

```yaml "--pull-request-reviews"
- run: npx --yes genaid run --pull-request-reviews
  env: ...
```

GenAID will automatically try to ignore duplicate review comments and only create new ones.
