...
sidebar:
  order: 0
description: Start developing with the GenAID VS Code Extension to create
...
import GenAIDCli from "../../../components/GenAIDCli.astro"

**GenAID** (Generative Authorative Intelligent Darbot Script, aka "degen script") is a self prompting, self learning, self healing, efficient generative AI darbot scripting language. Prompting is coding—store code as simple prompts with known expected calculated outputs based off common variables.

GenAID is a scripting language that integrates LLMs into the scripting process using a simplified JavaScript syntax.
Supported by our VS Code GenAID extension, it allows users to create, debug, and automate LLM-based scripts.

<YouTube id="https://youtu.be/ENunZe--7j0" posterQuality='high' />

## Preamble

Before you start writing GenAIDs, you will need to configure your environment to have access to a LLM.
The [configuration](/genaid/getting-started/configuration) covers this topic in details as they are a lot of options to consider.

:::tip

If you are running GenAID from a [GitHub CodeSpaces](https://github.com/features/codespaces), you can skip the configuration step as the extension will automatically use [GitHub Models](/genaid/getting-started/configuration#github).

:::

## Hello World

A GenAID is a JavaScript program that builds an LLM which is then executed by the GenAID runtime.
Let's start with a simple script that tells the LLM to generate a poem. In typical use, GenAID files
have the naming convention `<scriptname>.genai.mjs` and are stored in the `genaid` directory
...
In practice, your script may also import [system scripts](/genaid/reference/scripts/system) (automatically or manually specified) that add more messages to the requests.
...
GenAIDs can be executed from the [command line](/genaid/reference/cli) or run with a right-click context
menu selection inside Visual Studio Code. Because a GenAID is just JavaScript,
the execution of a script follows the normal JavaScript evaluation rules.
Once the script is executed, the generated messages are sent to the LLM server, and the response is processed by the GenAID runtime.

<GenAIDCli args="run poem" />
...
GenAID supports extracting structured data and files from the LLM output as we will see later.
...
GenAIDs support a way to declare [prompt variables](/genaid/reference/scripts/context), which allow to include content into the prompt and to refer to it later in the script.
...
GenAID are meant to work on a file or set of files. When you run a script in Visual Studio Code on a file or a folder, those files are passed to the script using the `env.files` variable. You can use this `env.files` to replace hard-coded paths and make your scripts
more resuable.
...
<GenAIDCli args='run summarize "**/*.md"' />

## Processing outputs

GenAID processes the outputs of the LLM and extracts files, diagnostics and code sections when possible.
...
the GenAID runtime interprets based on what the prompt requested from the model:
...
Our GenAID library parses the LLM output, interprets it, and in this case will
...
Of course, things can get more complex - with functions, schemas, ... -, but this is the basic flow of a GenAID script.
If you're looking for an exhaustive list of prompting techniques, checkout [the prompt report](https://learnprompting.org/).

## Using tools

[Tools](/genaid/reference/scripts/tools) are a way to register JavaScript callbacks with the LLM, they can be used
execute code, search the web, ... or read files!
Here is an example of a script that uses the [`fs_read_file`](/genaid/reference/scripts/system#systemfs_read_file) tool to read a file and summarize it:
...
You can add one more layer of indirection and use [agent_fs](/genaid/reference/scripts/system#systemagent_fs), a file system [agent](/genaid/reference/scripts/agents), to read the file. The agent combines a call to an LLM, and a set of tools related to file system queries.
...
While GenAIDs can be written with any IDE and run from the command line,
users of the [extension in Visual Studio Code](/genaid/getting-started/installation)
greatly benefit from the additional support for writing, debugging, and executing
GenAID provided. We strongly recommend starting by installing the extension.
