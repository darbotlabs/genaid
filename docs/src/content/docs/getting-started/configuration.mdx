...
You will need to configure the LLM connection and authorization secrets. You can use remote (like OpenAI, Azure, etc.) and local models (like Ollama, Jan, LMStudio, etc.) with GenAID.
...
You can also use the `small`, `large`, `vision` [model aliases](/genaid/reference/scripts/model-aliases) to use the default configured small, large and vision-enabled models.
...
The model aliases can also be overridden from the [cli run command](/genaid/reference/cli/run),
or environment variables or configuration file. [Learn more about model aliases](/genaid/reference/scripts/model-aliases).

```sh
genaid run ... --model largemodelid --small-model smallmodelid
```

or by adding the `GENAID_MODEL_LARGE` and `GENAID_MODEL_SMALL` environment variables.

```txt title=".env"
GENAID_MODEL_LARGE="azure_serverless:..."
GENAID_MODEL_SMALL="azure_serverless:..."
GENAID_MODEL_VISION="azure_serverless:..."
```
...
## `.env` file and `.env.genaid` file

GenAID uses a `.env` file (and `.env.genaid`) to load secrets and configuration information into the process environment variables.
GenAID multiple `.env` files to load configuration information.
...
the `.env`, `.env.genaid` file will appear grayed out in Visual Studio Code.
...
- GenAID script loads the following `.env` files in order by default:
...
npx genaid ... --env .env .env.debug
```

- by setting the `GENAID_ENV_FILE` environment variable.

```sh
GENAID_ENV_FILE=".env.local" npx genaid ...
```

- by specifying the `.env` file location in a [configuration file](/genaid/reference/configuration-files).

```json title="~/genaid.config.yaml"
{
    "$schema": "https://darbotlabs.github.io/genaid/schemas/config.json",
...
of the genaid process with the configuration values.
...
```sh
OPENAI_API_KEY="value" npx --yes genaid run ...
```

- GitHub Action configuration

```yaml title=".github/workflows/genaid.yml"
run: npx --yes genaid run ...
env:
...
The [configure](/genaid/reference/cli/configure) command is an interactive command to configure and validate the LLM connections.

```sh
npx genaid configure
```
...
Use `GENAID_MODEL_LARGE` and `GENAID_MODEL_SMALL` in your `.env` file to set the default model and small model.

```txt
GENAID_MODEL_LARGE=openai:gpt-4o
GENAID_MODEL_SMALL=openai:gpt-4o-mini
```
...
You can enable the `genaid:openai` and `genaid:openai:msg` [logging namespaces](/genaid/reference/scripts/logging) for more information about the requests and responses:
...
```yaml title="genaid.yml" "models: read"
permissions:
    models: read
```
...
Pass the `GITHUB_TOKEN` when running `genaid`

```yaml title="genaid.yml" "GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}"
run: npx -y genaid run ...
env:
...
system prompts. GenAID handles this internally.
...
You can also use a service principal as documented in [automation](/genaid/getting-started/automating-scripts).
...
In order to allow GenAID to list deployments in your Azure OpenAI service,
...
Set `NODE_ENV` to `development` to use the `DefaultAzureCredential` with the GenAID.
...
GenAID maintains a [default API version](https://learn.microsoft.com/en-us/azure/ai-services/openai/api-version-deprecation) to access Azure OpenAI.
...
it is included here. It allows you to do [vector search](/genaid/reference/scripts/vector-search) of your documents
...
GenAID uses the [OpenAI compatibility](https://ai.google.dev/gemini-api/docs/openai) layer of Google AI,
...
- [fallback tools](/genaid/reference/scripts/tools#fallbacktools) are enabled
...
GenAID will be able to leverage those [language models](https://code.visualstudio.com/api/extension-guides/language-model) as well.
...
or set the **GenAID > Language Chat Models Provider** setting to true.
This will default GenAID to use this provider for model aliases.
...
Confirm that you are allowing GenAID to use the GitHub Copilot Chat models.
...
The mapping of GenAID model names to Visual Studio Models is stored in the settings.
...
To use Anthropic models with GenAID, follow these steps:
...
GenAID assumes that you have configured AWS credentials in a way that the [AWS Node SDK will recognise](https://docs.aws.amazon.com/sdk-for-javascript/v3/developer-guide/setting-credentials-node.html).
...
To use Hugging Face models with GenAID, follow these steps:
...
You can enable the `genaid:anthropic` and `genaid:anthropic:msg` [logging namespaces](/genaid/reference/scripts/logging) for more information about the requests and responses:
...
GenAID is currently using the OpenAI API compatibility layer of Ollama.
...
GenAID will automatically pull the model, which may take some time depending on the model size. The model is cached locally by Ollama.
...
GenAID assumes the local server is at `http://localhost:1234/v1` by default.
...
- GenAID uses the [LMStudio CLI](https://lmstudio.ai/docs/cli)
...
By default, GenAID will set the site URL and name to `GenAID` but you can override these settings with your own values:
...
This `whisperasr` provider allows to configure a [transcription](/genaid/reference/scripts/transcription)
...
You can set the `HTTP_PROXY` and/or `HTTPS_PROXY` environment variables to run GenAID behind a proxy.
...
You can check your configuration by running the `genaid info env` [command](/genaid/reference/cli).
It will display the current configuration information parsed by GenAID.
...
Write your [first script](/genaid/getting-started/your-first-genaid).
