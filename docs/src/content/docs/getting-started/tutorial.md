...
sidebar:
  order: 100
description: Learn how to use GenAID with this interactive tutorial
  notebook featuring executable JavaScript code blocks.
keywords: tutorial, GenAID notebook, interactive learning, JavaScript, markdown
hero:
  image:
...
This Notebook is a GenAID tutorial. It is a Markdown document where each JavaScript code section is a runnable GenAID. You can execute each code block individually and see the results in the output section below the code block. To open this notebook in Visual Studio Code, press **F1** and run **GenAID: Create GenAID Markdown Notebook**.

Follow the steps in [configuration](https://darbotlabs.github.io/genaid/getting-started/configuration) to set up your environment and LLM access.
...
In GenAID, the [`env.files`](https://darbotlabs.github.io/genaid/reference/scripts/context/#environment-env) variable contains the [list of files in context](https://darbotlabs.github.io/genaid/reference/script/files), which can be determined by a user selection in the UI, CLI arguments, or pre-configured like in this script. You can change the files in `env.files` by editing the `files` field in the front matter at the start of the document.
...
$`Summarize https://raw.githubusercontent.com/darbotlabs/genaid/main/README.md in 1 sentence.`
```
