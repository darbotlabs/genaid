...
description: Suggestions for using GenAIDs more effectively
...
## GenAID allows chatbot users to create reusable scripts
...
GenAID allows you to write a prompt that is embedded in a JavaScript framework
...
GenAID allows users to add documents to their prompts. This
allows the LLM to have more background information related to the
task it is being asked to do.
In a GenAID, the JavaScript [`def`](/genaid/reference/scripts/context) command gives the
LLM the contents of a document and defines a name that
can be used in the prompt to refer to that document.
Standard document formats, like [pdf](/genaid/reference/scripts/pdf)
and [docx](/genaid/reference/scripts/docx) are supported so you just have
to name the files and our libraries will extract the text automatically.
You can parameterize the
input context further using [`env.files`](/genaid/reference/scripts/context).

## Focus a GenAID on having the LLM do 1 thing well

Say I wanted to use a GenAID to write a white paper. Instead of asking the model to
...
## Use the output of 1 GenAID as input to another
...
interface to the scripts [CLI](/genaid/reference/cli) and using
...
how to connect many of these with GenAID in [configuration](/genaid/getting-started/configuration).
