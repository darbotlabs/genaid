...
description: Learn how to configure content validation for Model Context
  Protocol (MCP) tools in GenAID Script.
tags:
  - mcp
  - validation
  - tools
...
excerpt: GenAID has introduced updates to enhance the security of Model
...
GenAID added a few feature to secure Model Context Protocol (MCP) tools and mitigate specific attacks such as rug pull, tool poisoning, or prompt injection.

Starting with `v1.127`, you can configure the following options as [documented here](/genaid/reference/scripts/mcp-tools#security):
...
- prompt injection detect using [content safety scanner](/genaid/reference/scripts/content-safety). This will scan both the tools definition file, to prevent **tool poisoning** and every tool output,
