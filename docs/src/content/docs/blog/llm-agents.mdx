...
tags:
  ...
  - user-input
  - questions
canonical_url: https://darbotlabs.github.io/genaid/blog/llm-agents
cover:
...
excerpt: GenAID redefines how agents interact with users by integrating
...
GenAID defines an [**agent**](/genaid/reference/scripts/agents) as a [tool](/genaid/reference/scripts/tools) that
runs an [inline prompt](/genaid/reference/scripts/inline-prompts) to accomplish a task. The agent LLM is typically augmented with
additional tools.
...
You can find the full script on GitHub right [here](https://github.com/darbotlabs/genaid/blob/main/packages/cli/genaid/system.agent_user_input.genai.mts).
...
GenAID will automatically append a description of all the tools used by the agent prompt so you don't have to worry about that part in the description.
...
The last argument is a set of model options, similar to [runPrompt](/genaid/reference/scripts/inline-prompts), to configure the LLM call made by the agent.
...
The agent is used like any other [tool](/genaid/reference/scripts/tools) by referencing it in the `script` options.
