...
tags:
  ...
  - webview
  - vscode
canonical_url: https://darbotlabs.github.io/genaid/blog/webview
description: Announcing the new GenAID view.
cover:
  alt: The image illustrates a split screen in an 8-bit style, contrasting two
    environments for the GenAID interface. On the left, a computer monitor
    ...
    context.
  image: ./webview.png
excerpt: We've enhanced the GenAID experience by moving beyond Visual
...
In the past, our Visual Studio Code visualization has relied on the built-in Markdown preview feature. It's been working great but sometimes it's not enough. We wanted to provide a more interactive experience for our users. So we decided to build a custom webview for GenAID.
...
![A screenshot of the GenAID view.](./webview.png)
...
As a result of this change, you can now access the GenAID view outside of Visual Studio Code. This means you can now **run** your scripts in a browser or any other webview-capable application.

Launch the [serve](/genaid/reference/cli/serve) command from the [cli](/genaid/reference/cli) to start the server and follow the instructions to open the view in your browser.
