...
tags:
  - video
  - ffmpeg
canonical_url: https://darbotlabs.github.io/genaid/blog/video
...
excerpt: Analyze videos with ease. Traditional LLMs require timestamp-based
  screenshots, often leading to context window issues. GenAID simplifies
...
Choosing those timestamp could be a challenge since you will run out of context window. GenAID provides a helpers to solve this tedious tasks around video analysis using LLMs.

- visit the [documentation](/genaid/reference/scripts/videos).

## tools and agents

We also provides wrap the new functionalities in [tools](/genaid/reference/scripts/tools) and [agents](/genaid/reference/scripts/agents) so you can use them in your scripts.
