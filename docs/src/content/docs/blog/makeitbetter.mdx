...
tags:
  ...
  - code improvement
  - automation
canonical_url: https://darbotlabs.github.io/genaid/blog/improve-your-code-twice-with-genai
cover:
...
excerpt: Harnessing the power of the `makeItBetter` function in GenAID
...
GenAID comes with a helper that tells the LLM to "make it better".
...
This line imports the `makeItBetter` function from the GenAID runtime. This function is used to improve code by repeating a set of instructions multiple times.
...
To run this script using the GenAID CLI, you need to execute the following command in your terminal:

```bash
genaid run makeitbetter
```

For detailed instructions on installing and setting up the GenAID CLI, check out the [GenAID documentation](https://darbotlabs.github.io/genaid/getting-started).
