...
````yaml
- name: Run GenAID on the TLA+ specs that are added in this pull request.
  run: npx --yes genaid run tlAI-Linter.genai.js $(git diff --name-only HEAD^ | grep '.tla') -oa results.sarif
